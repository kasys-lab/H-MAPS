...
- Fix: Removed window resizing logic completely
"""
...
class PipelineWorker(QObject):
    ...
    def __init__(self, ocr: demo_backend.OCRWrapper, retriever: demo_backend.S2ORCRetriever, args, memory_manager):
        super().__init__()
        self.ocr = ocr
        ...
        try:
            ...
            if not is_continuous:
                ...
                if len(self.accumulated_text_buffer) > LOCAL_CONTEXT_CHUNK_SIZE:
                    ...
                    self.memory.save_local_context(summary, self.current_scene_hash)
                    try:
            ...
            if current_time - self.last_session_update > SESSION_UPDATE_INTERVAL_SEC:
                ...
                recent_locals = self.memory.get_recent_local_contexts(limit=10)
                if recent_locals:
                    session_summary = demo_backend.llm_integrate_session(recent_locals)
                    self.memory.save_session_context(session_summary)
                    current_profile = self.memory.get_inferred_profile()
                    new_profile = demo_backend.llm_update_profile(current_profile, session_summary)
                    if new_profile != current_profile:
                        self.memory.save_inferred_profile(new_profile)
    ...
    def _fire_query(self, ocr_text: str, mode: str):
        ...
        # --- Phase 1: Context Preparation (Sanitize & Local Cache Search) ---
